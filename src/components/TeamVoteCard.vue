  ...
  <div class="team-vote-card" @click="handleCardClick">
    ...
    <div class="card-content">
      ...
      <button class="like-button" @click.stop="toggleLike">
        <img 
          v-if="card.liked"
          src="@/assets/投票/点赞（已选中状态）.png" 
          alt="已点赞" 
          class="like-icon"
        />
        <img 
          v-else
          src="@/assets/投票/点赞（未选中状态）.png" 
          alt="点赞" 
          class="like-icon"
        />
      </button>
...
.team-vote-card {
  ...
  max-width: 400px !important; /* 限制单张卡片的最大宽度，根据设计图估算 */
}

.card-image-wrapper {
  width: calc(100% - 30px); /* 左右各留出10px空间 */
  margin: 10px; /* 上下左右都有10px边距 */
  padding-top: 55%; /* 图片宽高比 */
  position: relative;
  overflow: hidden;
  border-radius: 5px;
}
...
.card-content {
  padding: 0 10px 10px 10px !important; /* 左右10px，下方10px，顶部0px */
  display: flex;
  flex-direction: column;
  gap: 8px !important; /* 内容项之间的间距 */
}
...
.card-details {
  display: flex;
  justify-content: space-between; /* 作者和票数左右对齐 */
...
@media (max-width: 600px) {
  .card-content {
    padding: 0 8px 8px 8px !important; /* 移动端减少padding */
  }
  .card-image-wrapper {
    width: calc(100% - 16px); /* 移动端左右各留出8px空间 */
    margin: 8px; /* 移动端边距 */
  }
  .card-title {
    font-size: 15px;
  }
  .card-author, .card-votes {
    font-size: 13px;
  }
  .like-button {
    padding: 6px 0;
